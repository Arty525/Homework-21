...
int main() {
	...
	if (command == "load") {
		...
		std::ifstream file("D:\\RTY\\GITHUB\\Repositories\\homework-21\\EX4\\save.bin", std::ios::binary);
		load(file, p, enemies);
		f[p.y].npc[p.x] = 'P';
		for (int i = 0; i < 5; ++i) {
	...
	do {
		...
		std::cin >> command;
		if (command == "save") {
			std::ofstream file("D:\\RTY\\GITHUB\\Repositories\\homework-21\\EX4\\save.bin", std::ios::binary);
			save(file, p, enemies);
			for (int i = 0; i < 5; ++i) {
